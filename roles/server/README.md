...
| zbxserver_vault_database_mount_point | <p>The HashiCorp Vault KV2 mount point where the Zabbix server database secret is stored.</p> | str | no |  | zabbix |
| zbxserver_vault_database_path | <p>The path within the HashiCorp Vault KV2 mount point where the Zabbix server database secret is stored.</p><p>The secret at this path must have the keys `username` and `password` defined.</p> | str | no |  | database |
...
| zbxserver_start_preprocessors | <p>The number of preprocessor processes to start.</p><p>Valid values are between 0 and 1000.</p><p>Reference: https://www.zabbix.com/documentation/current/en/manual/appendix/config/zabbix_server#startpreprocessors</p> | int | no |  | 3 |
...
| zbxserver_vault_users_mount_point | <p>The HashiCorp Vault KV2 mount point where the Zabbix server user secrets are stored.</p> | str | no |  | zabbix |
| zbxserver_vault_users_base_path | <p>The base path within the HashiCorp Vault KV2 mount point where the Zabbix server user secrets are stored.</p><p>The value should not include a trailing slash.</p> | str | no |  | users |
...
| zbxserver_mssql_odbc_connections | <p>A list of ODBC entries for Microsoft SQL Server.</p> | list of dicts of 'zbxserver_mssql_odbc_connections' options | no |  |  |
...
### Options for zbxserver_mssql_odbc_connections
|Option|Description|Type|Required|Choices|Default|
|---|---|---|---|---|---|
| name | <p>The name of the ODBC entry.</p> | str | yes |  |  |
| driver | <p>The driver to use for the ODBC entry.</p> | str | no |  | ODBC Driver 18 for SQL Server |
| uri | <p>The server URI for the ODBC entry.</p> | str | yes |  |  |
| encrypt | <p>Whether to encrypt the connection.</p> | bool | no |  | true |
